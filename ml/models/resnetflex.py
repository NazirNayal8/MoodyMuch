"""
Taken from official Pytorch Implementation:
...
class BasicBlock(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        """
        Expects input tensor of shape (N,C,H,W), where:
            - N: batch size
            - C: channel dimension
            - H: image height
            - W: image width
...
def make_layer(block: Type[Union[BasicBlock, Bottleneck]], inplanes: int, planes: int, blocks: int,
               norm_layer: nn.Module = nn.BatchNorm2d, stride: int = 1, dilate: bool = False) -> nn.Sequential:
    """
    Creates a multi-cnn layer using ResNet Basic Blocks and Bottle Neck blocks. The output of this
    layer represents a single stage in ResNet. 
    ...
    # Downsampling is used to adjust the input identity to the output of the convolution
    # So that they are compatible in shape when added
    ...
    # First layer is appended separately because it applies the stride 
    # provided in the arguments. Usually a stride signals the beginning
    # of a new stage in ResNet
